...
class Parser:
    ...
    def read(self, raw_file, ident_file):
        try:
            ...
            parser.logger.error(f"Couldn't read file. Exception:\n{traceback.format_exc()}")
        
        parser.logger.info(f"Read {len(self.psm_list)} from identification file")
    ...
    def read_raw_file(self, file_path):
        ...
        if extension.lower() == ".mzml":
            parser.logger.info(f"Inferred mzML format from {file_path}")
            return mzml.MzML(file_path)
        elif extension.lower() == ".mgf":
            parser.logger.info(f"Inferred MGF format from {file_path}")
            return mgf.MGF(file_path)
        else:
            parser.logger.error(f"Cannot infer format from {file_path}, only mzML and MGF formats are supported")
